...
public class Enemy : LivingEntity {
    ...
    public override void OnDamage(float damage, Vector3 hitPoint, Vector3 hitNormal) {
        if (!dead) {
            hitEffect.transform.position = hitPoint;//위치
            hitEffect.transform.rotation = Quaternion.LookRotation(hitNormal);//공격 방향
            hitEffect.Play();
    ...
    private void DisplayHPbar() {
        HpBar.rectTransform.localScale = new Vector3(health/startingHealth,1f,1f);
    }
}